...
GAME_DELAY = 1          # this variable is a constant, i have uses it to make the speed of the game abe lto be changed more easily by changin the number it increses the quiz length a small number makes the quiz faster and a big one makes it slower 1 = 1 second 
correct_answers = 0     # this sets the correct answers to 0 for the end of the quiz to show you how many correct answers you got overall
...
# i have made the code ask for the whole name and only print the first name is to make the code for user frendily
...
print("Thank you "+first_name)                          # this uses the list made before and prints it after saying thank you then the first name 
time.sleep(GAME_DELAY/2)         
...
if age >=14 :                                                                         # this is the start of the verification checking if the participant is too old or too young #
    ...
    time.sleep(GAME_DELAY*2) 
        ...
        # these are the questions and there options stored in one big list i have made the code split each question and option on the @ symbol is too make it more user frendily and accessible
    ...
    for i in range(len(questions)):    # this function allows the program to access each item in the lists above 
            ...
            while guesses<3:    # this section checks the answer to see if its correct or has invalid characters #
                ...
                if question_ask == answers[i]:                                                                   #
                    correct_answers += 1    # this line is where the code adds one point per question to the correct answer
                    if guesses <1:                                                                               #  
                        score += 10                                                                              #
