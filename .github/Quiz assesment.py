...
if AGE >=14 :
    ...
    time.sleep(1)
    QUESTIONS = ["Someone sends you a text that is hurtful and makes you feel bad about yourself. What should you do? A- Delete the message and try to forget about it. B- Keep the text ans show an adult you trust. C- Text the person back saying somthing mean to them. ",
    ...
    for i in range(len(QUESTIONS)):
            #print(i)
            print(QUESTIONS[i])
            QUESTIONASK=input("What do you think A, B or C? ")
            if QUESTIONASK == ANSWERS[i]:
                 print("well done "+FIRSTNAME+" your score is {Add score here}")
                 time.sleep(1)
            else:
                 print("try again")
